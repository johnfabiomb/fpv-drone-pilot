...
import { Difficulty, Island, Location } from '@core/models';

export { Difficulty } from '@core/models';
...
export class RouteBuilderService {
  ...
  buildPlan(input: PlanInput, anchorLocation?: Location): ItineraryDay[] {
    ...
    const filteredPool = (locations as Location[]).filter(loc => {
      ...
      if (difficulty === Difficulty.Easy) return loc.difficulty === Difficulty.Easy;
      if (difficulty === Difficulty.Moderate) return loc.difficulty === Difficulty.Easy || loc.difficulty === Difficulty.Moderate;
      return true;
    });
    ...
    for (let d = 0; d < days; d++) {
      ...
      if (d === 0 && anchorLocation) {
        ...
        const remaining = pool.filter(loc => {
          ...
          if (cominoUsed && this.getIsland(loc) === Island.Comino) return false;
          return true;
        });
      ...
      if (anchorIsland === Island.Comino) {
        cominoUsed = true;
        pool.forEach(loc => {
          if (!usedIds.has(loc.id) && this.getIsland(loc) === Island.Comino) usedIds.add(loc.id);
        });
      }
    }
    ...
    const grouped =
      anchorLocation && this.getIsland(anchorLocation) === Island.Gozo
  ...
  private getDayIsland(day: ItineraryDay): Island {
    return day.stops.length > 0 ? this.getIsland(day.stops[0].location) : Island.Malta;
  }

  private getIsland(loc: Location): Island {
    const tags: string[] = loc.tags || [];
    if (tags.includes('comino')) return Island.Comino;
    if (tags.includes('gozo')) return Island.Gozo;
    return Island.Malta;
  }
