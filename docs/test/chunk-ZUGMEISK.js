...
var BookingDataService = class _BookingDataService {
  ...
  getBooking(id) {
    return __async(this, null, function* () {
      const { data } = yield bookingsDb.from("bookings").select("id, org_id, booking_ref, staff_id, service_id, client_id, title, description, start_at, end_at, price_total, location, notes, status, allow_card, allow_inperson, deposit_percent, deposit_allowed").eq("id", id).maybeSingle();
      return data ?? null;
    });
  }
  ...
  syncCalendar() {
    return __async(this, null, function* () {
      ...
      if (error) {
        ...
        const { pushed = 0, pulled = 0, push_errors = [] } = data ?? {};
        const parts = [];
        if (pushed)
        ...
        if (push_errors.length)
          parts.push(`${push_errors.length} error(s)`);
...
//# sourceMappingURL=chunk-ZUGMEISK.js.map
