...
// src/app/shared/services/route-builder.service.ts
var STOPS_PER_DAY = {
  ...
  buildPlan(input, anchorLocation) {
    ...
    let pool = locations.filter((loc) => {
      if (anchorLocation && loc.id === anchorLocation.id)
        return false;
      if (difficulty === "easy")
        return loc.difficulty === "easy";
      if (difficulty === "moderate")
        return loc.difficulty === "easy" || loc.difficulty === "moderate";
      return true;
    });
    pool = pool.map((loc) => {
      let score;
    ...
    for (let d = 0; d < days; d++) {
      ...
      if (d === 0 && anchorLocation) {
        ...
        const remaining = pool.filter((loc) => {
          if (usedIds.has(loc.id))
            return false;
          if (cominoUsed && this.getIsland(loc) === "comino")
            return false;
          return true;
        });
      ...
      if (anchorIsland === "comino") {
        cominoUsed = true;
        pool.forEach((loc) => {
          if (!usedIds.has(loc.id) && this.getIsland(loc) === "comino")
            usedIds.add(loc.id);
        });
      }
    }
    ...
    const grouped = anchorLocation && this.getIsland(anchorLocation) === "gozo" ? [...gozoDays, ...maltaDays, ...cominoDays] : [...maltaDays, ...cominoDays, ...gozoDays];
...
// src/app/platform/route-builder/route-builder.component.ts
...
var RouteBuilderComponent = class _RouteBuilderComponent {
  constructor(router, activatedRoute, routeBuilderService) {
    this.router = router;
