...
function getExperiencesNearLocation(location, providers) {
  return getAllExperiences(providers).map((entry) => __spreadProps(__spreadValues({}, entry), {
    dist: nearestSpotKm(location, entry.experience),
    tagged: entry.experience.spots.some((s) => s.nearLocationIds?.includes(location.id))
  })).filter(({ tagged, dist }) => tagged || dist <= NEAR_LOCATION_KM).sort((a, b) => Number(b.tagged) - Number(a.tagged) || a.dist - b.dist).map(({ experience, provider }) => ({ experience, provider }));
}
...
//# sourceMappingURL=chunk-ETA2JZSR.js.map
